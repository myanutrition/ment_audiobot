...
TEXTS_BEFORE = {
    ...
    3: (
        ...
        "говорит «я вечером срываюсь на сладкое», за этим может стоять сотни причин 🍫\n\n"
        "Это усталость, запреты, тревога, отсутствие отдыха, страх снова сорваться, ощущение, что сладкое "
        "осталось единственным способом прожить день. Если специалист сразу даёт совет, он может не попасть "
        "в настоящую причину, чаще так и происходит."
    ),
    4: (
...
TEXTS_AFTER = {
    ...
    2: (
        ...
        "• Где повторяется главная трудность?\n"
        "• Какой первый фокус можно выбрать?\n"
        "• Какое действие человек попробует до следующей встречи?\n\n"
    ...
    5: (
        "После аудио возьмите один тезис про питание, который недавно встретили в соцсетях, и проверьте его "
        "по вопросам:\n\n"
        "• Кто это сказал?\n"
        "• На чём основано утверждение?\n"
        "• Это мнение, реклама, исследование или личный опыт?\n"
        "• Кому это применимо?\n"
        "• Какие есть ограничения?\n"
        "• Можно ли использовать это в работе с клиентом?\n"
    ...
    7: (
        ...
        "• Сессии каждую неделю\n"
        "• Сопровождение реального клиента\n\n"
...
TEXT_FINAL = (
    ...
    "• умение упрощать\n"
    "• умение быть собой\n\n"
    "Это то, что даёт клиентам ощущение, что их понимают, слышат и хотят помочь.\n\n"
...
async def daily_job_core(bot):
    """Сама логика рассылки - без проверки часа. Можно вызывать вручную (например, /runnow)."""
    users = get_all_users()
    ...
    for user_id, start_date_str, last_day in users:
        ...
        try:
            ...
            # ">=" вместо "==" - если по какой-то причине рассылка была пропущена
            # в свой день (сбой сервера, сеть и т.п.), пользователь всё равно получит
            # следующий день при первой же успешной проверке, а не застрянет навсегда.
...
async def daily_job(context: ContextTypes.DEFAULT_TYPE):
    """Обёртка для планировщика - проверяет час перед тем, как запускать рассылку."""
    if datetime.now().hour != SEND_HOUR:
        return
    await daily_job_core(context.bot)
...
async def runnow_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_CHAT_ID:
        return
    await update.message.reply_text("🚀 Запускаю рассылку прямо сейчас (принудительно, вне зависимости от часа)...")
    await daily_job_core(context.bot)
...
def main():
    ...
    # Проверяем каждые 5 минут, а не один раз в сутки в конкретную секунду.
    # Сама daily_job решает, подходящий ли сейчас час (SEND_HOUR) для отправки,
    # и самостоятельно "догоняет" пропущенные дни. Это защищает от ситуации,
    # когда бот перезапускался/падал именно в момент планового запуска.
    job_queue.run_repeating(daily_job, interval=300, first=10)
